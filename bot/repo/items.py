...
def _not_equipped(player: Player) -> tuple[str, list[int]]:
    ids = [i for i in (player.weapon_id, player.armor_id, player.amulet_id) if i is not None]
    if not ids:
        return "", []
    return f" AND id NOT IN ({','.join('?' * len(ids))})", ids


async def count_sellable(db: aiosqlite.Connection, player: Player) -> int:
    clause, ids = _not_equipped(player)
    async with db.execute(
        f"SELECT COUNT(*) FROM items WHERE owner_id = ?{clause}", [player.user_id, *ids]
    ) as cur:
        row = await cur.fetchone()
    return int(row[0]) if row is not None else 0


async def sellable_page(
    db: aiosqlite.Connection, player: Player, limit: int, offset: int
) -> list[Item]:
    """Solo lo que no lleva puesto: el equipo se desequipa antes de venderlo."""
    clause, ids = _not_equipped(player)
    sql = (
        f"SELECT {_COLUMNS} FROM items WHERE owner_id = ?{clause} ORDER BY id DESC LIMIT ? OFFSET ?"
    )
    async with db.execute(sql, [player.user_id, *ids, limit, offset]) as cur:
        rows = await cur.fetchall()
    return [_to_item(row) for row in rows]
